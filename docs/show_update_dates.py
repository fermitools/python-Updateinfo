...
'''
    This script will simply print out updates and their publication dates
'''
...
if __name__ == '__main__':
    ...
    PARSER.add_argument('--valsep', default='|',
                        help='How is output seperated')
    ...
    for UPDATE in MYUPDATEINFO:
        TXT = MYUPDATEINFO[UPDATE].updateid + ARGS.valsep + MYUPDATEINFO[UPDATE].updatetype + ARGS.valsep
        TXT = TXT + str(datetime.strftime(MYUPDATEINFO[UPDATE].issued_date, '%d %b %Y 00:00:00')) + ARGS.valsep
        for COL in MYUPDATEINFO[UPDATE].collections:
            for PKG in MYUPDATEINFO[UPDATE].collections[COL]:
                print(TXT + MYUPDATEINFO[UPDATE].collections[COL][PKG].name + ARGS.valsep + MYUPDATEINFO[UPDATE].collections[COL][PKG].version + '-' + MYUPDATEINFO[UPDATE].collections[COL][PKG].release + '.' + MYUPDATEINFO[UPDATE].collections[COL][PKG].arch)
